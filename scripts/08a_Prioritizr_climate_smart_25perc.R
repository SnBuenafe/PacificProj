...
represent_feat <- bind_cols(features_list_SSP126, feat.nested) #just using features_list_SSP126, since it's the same for every climate scenario anynway.
colnames(represent_feat)[1] <- 'features'
write_csv(represent_feat, 'outputs/08_Prioritizr/08a_25perc/summary_represent_feat.csv')
...
write_csv(summary_stat, 'outputs/08_Prioritizr/08a_25perc/summary_stat.csv')
...
ggsave('outputs/08_Prioritizr/08a_25perc/subset_plans.png', gg_subset, width = 29.7, height = 21, dpi = 300)
...
gg_no.regret_subset <- gg_no.regret_target_90 + gg_no.regret_target_60 + gg_no.regret_target_30 +
  plot_layout(nrow = 1, guides = 'collect') +
  plot_annotation(tag_levels = 'A', tag_suffix = ')')
ggsave('pdfs/08_Prioritizr/08a_25perc/no.regret.plan_subset.pdf', gg_no.regret_subset, width = 15, height = 5, dpi = 300)
ggsave('outputs/08_Prioritizr/08a_25perc/no.regret.plan_subset.png', gg_no.regret_subset, width = 15, height = 5, dpi = 300)
