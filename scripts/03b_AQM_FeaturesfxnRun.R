...
#running with .shp
run02 <- features_pus(path = "outputs/AQM_wflow/02b_bycatch",
                      outdir = "outputs/AQM_wflow/03b_features/",
...
temp <- as.data.frame(run01) %>% rename(features = feature_names) %>% group_by(features) %>% summarize(total_area = sum(area_km2), percentage = (sum(area_km2)/total_PU_area)*100) 
print(temp, n = 1000)
