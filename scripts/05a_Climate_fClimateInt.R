...
fClimateInt <- function(input, scenario, inpdir, outdir, pu, ...) {
  ...
  if(input == "RCE") {
    ...
    pu_file <- pu_file %>% 
      ...
      dplyr::mutate(value = value*50) %>% # multiplying by 50 years to make RCE unitless
      dplyr::mutate(trans_value = kader:::cuberoot(value/50)) # transformed values are just for presenting the data in a figure.
