...
# path: folder's name where aquamaps.csv files are located
# outdir: where to put the .csv species files
# olayer: for what ocean layer do you want the species
# prob_threshold: threshold of probability to be considered (e.g. 0.5)
...
# Input Files
# 1. a lookup table of which species are found in each half-degree cell (and the "probability of occurrence" of that species in that cell)   = hcaf.csv
...
aqua_start <- function(path, outdir, olayer, prob_threshold, sp_env, type, region, ...) {
  ...
  if(stringr::str_detect(string = region, pattern = ".grd") == TRUE) {
    ...
    region <- resample(region, rs, resample = "ngb")
  } else if (stringr::str_detect(string = region, pattern = ".tif") == TRUE) { #using nearest neighbor as the method to compute values
    region <- readAll(raster(region))
    region <- resample(region, rs, resample = "ngb")
  }
  ...
  hspen <- fread(second_csv) %>% 
    dplyr::select(tidyselect::matches("Species|Pelagic|Depth|Oxy|Temp|Salinity|Rank")) %>% #only gets the rows with data on these variables
    dplyr::filter(Rank == sp_env) #filters only species with this rank
  # Reading the speciesoccursum file 
  speciesInfo <- fread(third_csv, fill = TRUE) #fills in blank cells
  ...
  IDs_df <- vector("list", length(speciesID)) #creates empty vector with the length of the number of species
  ...
  IDs_df <- foreach(i = 1:length(speciesID), .packages = c("raster", "data.table", "dplyr", "magrittr", "sf")) %dopar% { # if you dont add any combine argument it will return a list
    ...
    if(nrow(z) >= 10 & mean(z$CenterLat) != (z$CenterLat[1])) {
      ...
      rs1 <- rasterFromXYZ(as.data.frame(z)  #data.frame should have first column = Long, second column = Lat, third and other columns = other values
