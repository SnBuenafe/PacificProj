...
fFilterQuartile <- function(velocity_file, RCE_file, feature_prov, outdir, scenario, feature_n, data, prov, quantile_value...) {
  ...
  # Intersect conservation features with climate-smart features
    ...
    for(i in 1:length(list)) {
        ...
        qrt_RCE <- quantile(temp[[i]]$RCE)[quantile_value]
        qrt_velocity <- quantile(temp[[i]]$velocity)[quantile_value]
        
        temp_x[[i]] <- temp[[i]] %>% 
          dplyr::filter((RCE <= qrt_RCE) | (velocity <= qrt_velocity))
    }
    
    filter_PU_final <- do.call(rbind, temp_x)
    filter_PU_final <- filter_PU_final %>% 
      dplyr::select(-cellsID.1, -cellsID.2) %>% 
